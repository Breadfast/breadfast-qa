...
At the very start, create the story's own folder **directly under the repo root** — `<repo root>\<JIRA_TICKET_ID>\` — with the standard subfolders if it doesn't exist (`requirements-analysis/ figma-analysis/ hls/ browserstack/ testcases/ automation/ execution-reports/ screenshots/ defects/ evidence/`). **All** outputs for the story go here and are reused on any retest/update.

> **Story folders are NOT committed (changed 2026-08-10).** `/B10-*/` is gitignored in full. This
> repo carries the **workflow**; a run's output is local to whoever ran it. Anything that turns out
> to be **reusable** is promoted into `automation/` — it does not stay in a story folder. That rule
> exists because the old "track `B10-*/automation/`" convention let **nine** copies of
> `upload_browserstack.js` and **seven** of `gen_browserstack_csv.js` accumulate, and made STEP 7
> depend on files that disappear the moment a story folder is cleaned up. The shared BrowserStack
> tooling now lives once at [`automation/browserstack/`](automation/browserstack/). (The QA Platform derives this via `storyDir(jiraKey)` = companion/repo root; runtime data — SQLite DB, logs, browser sessions — stays in the separate workspace, never in the story folder.)
