...
6. **CONFORMANCE GATE — run [`framework-conformance`](../framework-conformance/SKILL.md) (subagent)
   before recording.** It reviews only the code this story authored against the framework's *real*
   conventions — the checks `mvn test-compile` and checkstyle cannot make (no private methods or data
   literals in the test class, no new type under `modals/`, no fluent builder, POM-owned fixture data,
   whole-token class matching, Material `contains()` labels, no `SoftAssert`, vacuous-pass guard,
   shared-tree hygiene). It writes `automation/conformance-review.md`, carries the same grounding
