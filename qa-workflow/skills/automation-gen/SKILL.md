...
> **v2.2 (2026-08-24):** **golden references must be human-authored** (contract §4.2) and the
> alignment self-review is now **measured**, not eyeballed (§7.1). Rationale: on B10-58603 a new mobile
> screen was matched against three siblings in its own package that had all been generated by earlier
> story runs, so the conformance review "passed" against generated code. The class shipped 3× the size
> of the human-authored equivalent with its own gesture and geometry code and 13 expected-value
> getters — while `mobile-native-framework.md` §2–§4 already documented the correct locator, method and
> scrolling patterns. **The rules existed; the process did not force reading them against the right
> files.** Phase contract unchanged; no new artifact key.
...
1. **Resolve + learn the framework** (contract §2–3): catalogs first, framework docs, the target
   ...
   **For mobile, `mobile-native-framework.md` §2 (locators), §3 (waits + the scroll helper) and §4
   (method conventions) are required reading before the first line** — they already answer the
   locator, method-naming and scroll-to-element questions, and skipping them is what produced the
   B10-58603 rework.
1.5 **Identify HUMAN-AUTHORED references (contract §4.2).** Establish authorship with
   `git log --format='%an' -- <path> | sort -u` before treating any file as a pattern to match. The
   framework contains generated code now; a file authored only by a story run is **not** a reference.
   Record the 2–3 closest human-authored implementations **with their authors** in
...
6. **CONFORMANCE GATE — run [`framework-conformance`](../framework-conformance/SKILL.md) (subagent,
   ...
   *real* conventions — the checks `mvn test-compile` and checkstyle cannot make (no private methods or
   ...
   `automation/conformance-review.md`, **includes the measured size/shape comparison against the
   human-authored references (contract §7.1)**, carries the same grounding
...
node qa-workflow/bin/qa-cli.js record "<storyDir>" automation \
     --path automation/README.md --generator automation-gen@2.2 --derive-artifacts testcases
```
> **Lock-seam note:** the `2.1` → `2.2` bump is a generator-version change, so
