...
1. **Authenticated Playwright browser session (PRIMARY)** — the reliable channel on this account: immune to **both** the Starter-plan REST quota and the MCP seat-cap. **Self-contained in this repo — NO qa-platform dependency:** reconnect script `qa-workflow/bin/figma-connect.js`, saved session `auth/figma-auth.json` (repo root, gitignored).
   - **Session gate (run FIRST):** `node qa-workflow/bin/figma-connect.js --status` — reads `auth/figma-auth.json` (override `FIGMA_AUTH_PATH`), fast-fails on the structural checks, then **makes one real authenticated request to Figma** and reports on *that*. One JSON line. **`0` = usable** (`FRESH` verified, or `UNVERIFIED` when the probe itself could not complete — offline/timeout/`429`; capture anyway, **do not re-login on that alone**), **`3` = reconnect** (`STALE` = file intact but Figma rejected it, plus `MISSING`/`INVALID`/`EXPIRED`), **`2`** = internal error, **`4`** = `UNVERIFIED` under `--strict`. `--offline` keeps the old date-only behaviour. Full contract and why a date is not proof of a session: [`docs/ai/testing-process.md`](../../../docs/ai/testing-process.md) §4.1.
     - **Not usable → notify the user and OPEN A BROWSER to reconnect** (never mark blocked): run `node qa-workflow/bin/figma-connect.js` from the repo root (`@playwright/test` resolves from the repo's own `node_modules`) — a headed Chromium opens on Figma login; the user signs in with Google; the script auto-detects success and writes the full `storageState` to `auth/figma-auth.json`. Resume capture once it exits 0.
     - **Fallback if no headed browser can launch** (script exits 2 / no display): reconnect **in-session via the Playwright MCP** — `browser_navigate` to `https://www.figma.com/login`, ask the user to sign in, poll until the URL leaves the login flow, then read `page.context().storageState()` (via `browser_run_code_unsafe`) and **round-trip it to disk with the one guarded writer** — write the JSON to a temp file and run `node qa-workflow/bin/figma-connect.js --import-state <that file>` (`-` reads stdin). It validates, writes **atomically**, refuses a logged-out jar, and probes the result. **Never hand-`Write` `auth/figma-auth.json`** — that step was documented before and demonstrably never happened (the file's mtime sat frozen for two weeks while the operator re-logged-in repeatedly).
   - **Capture runs through THIS repo's Playwright, not the MCP browser (changed 2026-09-08):**
     ```
     node automation/figma/capture_frames.js --spec <storyDir>/figma-analysis/frames/spec.json
     ```
     It restores the jar correctly, gates on the target file's own canvas, enforces the sha256-distinctness rule, and — the point of the change — **writes the session back after a run that proved authenticated**, so Figma's token rotation is banked and the 25-day window resets on **use** instead of on last manual login. Exits `0` all captured · `1` some frame failed · `2` bad usage · `3` no session / no file access.
     **The MCP browser cannot read `auth/figma-auth.json`** — it runs its own profile, so it always starts logged out. Driving capture through it is what made the operator re-login every single time. Use it for the reconnect fallback above and for interactive exploration; not as the capture channel.
   - **EXPORT-GRADE capture — `Ctrl+Shift+C` Copy as PNG (THE DEFAULT), as implemented by that script:** deep-link to the **frame node** (reliable selection) → wait for canvas (title contains "–") → `grantPermissions(['clipboard-read','clipboard-write'])` → **`Ctrl+Shift+C` (Copy as PNG)** → in `page.evaluate` read `navigator.clipboard.read()` for `image/png`. Renders the node at **2× natively, no editor chrome**. Beats plain viewport screenshots (chrome-laden, screen-resolution → **not** valid for pixel diffing) and the `Ctrl+Shift+E` export dialog (whose buttons aren't cleanly exposed to DOM automation). The shortcut must target the **canvas** (don't click the right panel first, or it copies text). Image-heavy frame? raise `settleMs` so lazy images fill, else mockups export **black**.
