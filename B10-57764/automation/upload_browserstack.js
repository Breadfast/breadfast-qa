...
/**
 * B10-57764 — upload the APPROVED test cases to BrowserStack Test Management.
 *
 * Destination is fixed and pre-created by the operator (verified live 2026-08-10):
 *   project PR-5 "BCard Squad" (/projects/2407303)
 *     └── folder 54229450 "Card Ops.Sprint3.4"
 *           └── folder 54229453 "Admin Portal  — Featured Perk Flag (1 per category)"  ← target, was empty
 *
 ...
 *   node upload_browserstack.js
 *   node upload_browserstack.js --verify-only
 */
...
const PROJECT_ID = 'PR-5';
const FOLDER_ID = '54229453';
...
(async () => {
  ...
  fs.writeFileSync(path.join(__dirname, 'tc-map.json'), JSON.stringify(map, null, 2), 'utf8');
  console.log(`\nwrote tc-map.json (${map.filter((m) => m.id).length}/${CASES.length} ids)`);
