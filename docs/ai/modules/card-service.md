...
**API clients**
- `helpers/apiClients/mobileApiClients/CardServiceApiClient` — **the single client for the whole Card ecosystem, mobile and admin panel alike.** Token, card status, wallet user id, set passcode/PIN, link/activate/replace card, balance, transaction history, invitation-code batch (generate/export/consume), transfers (`checkSender`/`receiverDetails`/`completeTransaction`), card pool, `changeCardStatus`, `collectCard`, plus the **perks** surface: read oracles (`getPerkField`/`getPerkAttribute`/`getAuthoredBranchLines`/`perkIsFeatured`/`countPerks`), shape-finders (`findSectionIdWithAtLeastTwoActivePerksAndNoFeaturedPerk`, `findActivePerkWithCouponType`, `findActivePerkIdsInSection`, …), `setPerkFeatured`/`featurePerkForSetup`, and `createDisposableActivePerkInSection`/`expirePerk`.
  - **One authentication, one token.** Every method above — perks included — takes the `CardService` model last and reads `cardService.getCardServiceToken()`. The perks endpoints authenticate with that same raw token (verified live 2026-08-20: raw token → 200/400/404, no `Bearer` prefix, 401 only when the header is absent). A second `loginAndGetJwtToken()` existed until 2026-08-20 in a separate `CardAdminPanelPerksApiClient`; both were removed and merged here.
