...
| **Provision a BCard user in status `Registered`** (no DB bastion) | `CardUserProvisioner.provisionRegisteredUser()` — wraps `CardUserFactory.provision()` and reads the registration OTP from the Google Chat space instead of `bf_phone_otp_verification`, whose SSH bastion is dead. `provisionUnregisteredUser()` gives the no-card state. **Needed whenever a customer-app surface is gated on card status** — the app's pickup-location screen, for instance, only appears during BCard sign-up, so an account that already holds a card can never reach it (B10-58669) | [helpers/CardUserProvisioner.js](../../../automation/helpers/CardUserProvisioner.js) |
| **Register / verify-OTP / create a card user via API** | `CardUserFactory.provision / registerOnly / collectViaApi / editKyc / linkCardApi / setPinApi / activateApi / status`. ⚠ The `wp-json` user endpoints require an `accept-version` **semver** header (`1.0.0`); without it every call answers `422 Validation Error` (fixed 2026-09-03) | [helpers/CardUserFactory.js](../../../automation/helpers/CardUserFactory.js) |
| **Pickup locations + temporary closures via API** | `CardServiceApiClient.listLocations / getLocationByName / getLocationClosures / locationIsOpenNow / getLocationClosedUntil / getWorkingHoursSlotSummaries / getLocationFieldSnapshot / createAlwaysOpenPickupLocation / createPartialHoursPickupLocation / clearLocationClosures / updateLocationClosures / activeOneTimeClosure` — `locations/list` is the **app-facing read model**: it carries `temporary_closures[]`, `open_now` and `temporarily_closed_until`, and there is no separate closure endpoint (B10-58669) | `helpers/apiClients/mobileApiClients/CardServiceApiClient.java` |
