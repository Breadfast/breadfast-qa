...
>
> **Status (2026-07-22): legacy execution engine.** Per the project's architecture decision, `docs/ai/QA_PROCESS.md` (repo root) is now the authoritative, platform-agnostic QA methodology, and Claude Code is the primary execution environment going forward. This platform receives only: completion of engineering work already in progress, certification/hardening, critical bug fixes, and migration support — no new platform-specific capabilities. Parity Certification, Story Health, Review Confidence, Run Lifecycle Management, and Session Continuity below are **QA capabilities**, not QA-Platform capabilities — they happen to be implemented here today; the migration goal is for them to become implementation-independent, executable by Claude Code directly under the Canonical QA Process, not tied to this codebase.
...
- **Session Continuity (Claude conversation), with the Context Builder guarantee kept as a fallback, not given up.** Consecutive nodes in a run share one `claude --resume` session (`Run.engineSession`; never shared across stories/runs), wired through `withSession()` in `apps/worker/src/nodes.ts` — the worker's seam for every `ai()`/`runAiTask`/`runClaude` call. Every node's prompt is still rebuilt in full every time from `buildRunContext()` (`packages/shared/src/context-builder.ts`), which reconstructs state purely from persisted, succeeded `RunStep.outputJson` rows — resuming only ever adds conversational continuity on top of that, it is never required for correctness. If `--resume` fails (wrong worker machine, an expired session, or one cleared by a Restart From Step), `SessionNotFoundError` (`packages/engine/src/claude-runner.ts`, an env-configurable text-pattern match like `UsageLimitError`) is caught and the call is retried once with no session — a brand-new one is then persisted going forward. `resetStepsFrom` clears `engineSession` on a `restart` (downstream turns it would still remember are being discarded) but leaves it intact on a plain `resume` (nothing downstream ever ran).
- **Session Continuity (app-under-test browser), a separate dimension from the Claude conversation above.** Every `runClaude`/`runAiTask` call gets its own fresh, isolated Playwright MCP browser process — this repo has no standalone/long-lived MCP server, and no config anywhere wires one up (`mcpConfig` on `ClaudeRunOptions` exists but has no call sites), so a genuinely continuous browser across nodes is NOT implemented. Instead, cookies + localStorage for the app under test are captured at the end of `exploratory_testing`'s probe and `execution`'s web run and restored at the start of the next one via `appSessionCredsStep()`/`appSessionSaveStep()` (`apps/worker/src/nodes.ts`), persisted per-story at `appSessionPath(jiraKey)` (`packages/shared/src/paths.ts`, under the workspace's `browser-sessions/<TICKET>/`) — mirroring the pre-existing Figma auth pattern (`figmaAuthRestoreStep()`), extended to localStorage and scoped per-story instead of per-user. Restoring an expired/invalid session falls back to an ordinary fresh login, detected by the agent landing on a login screen — never a hard dependency. In-memory JS runtime state (SPA store, current route, open WebSocket) does NOT survive across nodes under this design; only what's persisted to cookies/localStorage does. **This tier is complete and sufficient for the QA Platform — not a stepping stone to build further here.** A truly continuous browser process (matching in-memory state too) was considered and explicitly rejected as over-engineering a platform this project is moving away from (see CLAUDE.md's legacy-execution-engine framing). That requirement is instead captured as target architecture for the future Claude Code execution engine in [`docs/ai/execution-engine.md`](../docs/ai/execution-engine.md), since Claude Code (unlike this platform's per-node headless-subprocess architecture) can satisfy it naturally by running a story's whole QA Process as one continuous session.
...
- **Verification:** `scripts/verify-run-lifecycle.mjs` (same real-DB harness pattern as `phase1/2-validate.mjs`) — the Context Builder fallback guarantee is proven by calling `buildRunContext()` against a freshly-queried run, not anything held in the test's own process state; test 5 additionally asserts `engineSession` is cleared by a Restart From Step.
