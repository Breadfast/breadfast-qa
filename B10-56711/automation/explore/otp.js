 ...
 *
 * Message format in the space:  "OTP For +201188369495 is 1234\nENV: testing"
 */
const fs = require('fs');
const path = require('path');

// Cross-platform: resolve the Java framework checkout instead of assuming a drive letter.
// Override with BF_PROPERTIES_PATH, or QA_FRAMEWORK_PATH for the whole framework.
const PROPS =
  process.env.BF_PROPERTIES_PATH ||
  require('../../../automation/config/framework').environmentsFile('config_testing.properties');

function props() {
  if (!PROPS) {
    throw new Error(
      'otp.js: Java QA framework not located. Set QA_FRAMEWORK_PATH (folder containing pom.xml) ' +
      'or BF_PROPERTIES_PATH (config_testing.properties directly).'
    );
  }
  const out = {};
...
if (require.main === module) {
  const phone = process.argv[2] || '+201188369495';
  (async () => {
