 ...
 * capturing evidence at every step. This is the reusable path every B10-56711 scenario starts from.
 *
...
async function run(platform, locale, phone) {
  ...
  // ---- passcode (6 digits) ----
  // Marker taken BEFORE the passcode, because submitting it is what triggers the Pay-access OTP send.
  const payOtpMarker = await otp.marker(e164).catch(() => new Date(0).toISOString());
  ...
  // ---- Pay-access OTP ----
  // The screen reads "Enter the 4-digit verification code sent to <number>", i.e. the code is really
  // SENT — so it must be read from the Google Chat space, not derived. CLAUDE.md §7's "last 4 digits"
  // rule holds for the card APPLICATION and ACTIVATION OTPs, and it happened to coincide with the
  // last 4 digits on the B10-56717 account (…7660); it does NOT hold here. Deriving it sent the app
  // back to the passcode gate. Verified on +201188369495, 2026-07-30.
  let payOtp;
  try {
    const got = await otp.waitForOtp(e164, { after: payOtpMarker, timeoutMs: 90000 });
    payOtp = got.otp;
    log(`  Pay OTP ${payOtp} read from Google Chat (sent ${got.createTime})`);
  } catch (e) {
    payOtp = e164.slice(-4);
    log(`  Pay OTP not found in Chat (${e.message}) — falling back to last-4 ${payOtp}`);
  }
  ...
  // Tap ONLY when the interstitial is actually present. The old coordinate fallback fired
  // unconditionally on Android, and AND.notNow [539,2063] lands on the bottom-bar **Pay tab** once
  // Pay home is already showing — which re-enters Pay and bounces the app back to the passcode gate,
  // destroying the session's state. This account never gets the interstitial. (2026-07-30)
  const interstitialRows = await rows(sid);
  const notNow = interstitialRows.find((r) => /^(Not now|ليس الآن|لاحقا|لاحقاً)$/.test((r.label || r.name || r.text || '').trim()));
  const interstitialPresent = notNow
    || interstitialRows.some((r) => /faster checkout|Save card|حفظ البطاقة|الدفع الأسرع/i.test((r.label || r.name || r.text || '')));
  if (notNow) { await tapRow(sid, notNow, 'Not now'); await sleep(9000); }
  else if (interstitialPresent && !ios) { await tap(sid, AND.notNow[0], AND.notNow[1]); log('  tapped Not now by coordinate (interstitial detected, unlabelled)'); await sleep(9000); }
  else { log('  no "save card" interstitial on this account — skipped (blind tap would hit the Pay tab)'); }

  // ---- Pay home ----
  // Re-assert the Compose settings here: the Pay content area is Jetpack Compose and reports its
  // nodes as INVISIBLE, so without these the whole area collapses and "See all" is unreachable.
  // Applying them at session creation is not always enough — re-apply once the surface is rendered.
  if (!ios) {
